...
\hypersetup{
    ...
    pdfauthor={Daniel Alejandro Horna}
}
...
\begin{titlepage}
    ...
    \begin{flushleft}
        \large
        Studente: \textbf{Daniel Alejandro Horna}\\
        Matricola: \textbf{0001126664}\\
        Docente: \textbf{Prof. Luciano Margara}\\
...
dove \(nG=\mathcal{O}\). Il più piccolo intero positivo \(n\) tale che \(nG=\mathcal{O}\) è chiamato ordine del punto \(G\). Nei protocolli ECC, \(G\) è pubblico, mentre la chiave privata è uno scalare segreto \(d\). La chiave pubblica è
...
L'analogia tra i due contesti è la stessa già introdotta nel Capitolo 2: in entrambi i casi, un'operazione diretta è efficiente mentre il problema inverso è computazionalmente difficile.
...
dove \(nG=\mathcal{O}\). Dato un punto \(Q \in \langle G \rangle\), esiste un intero \(d\), con \(0 \leq d < n\), tale che
...
\section{Teorema di Hasse e cofattore}

Il numero totale di punti della curva \(E(\mathbb{F}_p)\) non è arbitrario. Il teorema di Hasse stabilisce che

\[
\left|\#E(\mathbb{F}_p)-(p+1)\right|\leq 2\sqrt{p}.
\]

Quindi il numero di punti della curva è dell'ordine di \(p\). L'ordine \(n\) del punto base \(G\) divide \(\#E(\mathbb{F}_p)\), e il quoziente

\[
h=\frac{\#E(\mathbb{F}_p)}{n}
\]

è chiamato cofattore~\cite{margara_ec,sec1}. Nei protocolli ECC si sceglie normalmente un punto base \(G\) di ordine primo grande e un cofattore piccolo. Questa scelta riduce il rischio di attacchi basati su sottogruppi piccoli e rafforza la validità dei parametri usati nel sistema.
...
Questa differenza mostra che RSA, Diffie--Hellman classico ed ECC si basano su problemi matematici distinti: rispettivamente fattorizzazione, logaritmo discreto modulo \(p\) ed ECDLP~\cite{margara_rsa,margara_dh,margara_ec,nist_sp800_57}. Un confronto sistematico tra questi sistemi è presentato nel Capitolo~9.
